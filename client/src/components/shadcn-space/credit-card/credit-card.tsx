...
const VisaLogo = () => (
  <span className="text-white font-bold italic text-lg tracking-tight" aria-label="Visa">
    VISA
  </span>
)
        ...
        >
          ...
          >
            ...
            <CardContent className="relative z-10 flex flex-col justify-between h-full p-0">
              {/* Row 1: issuer | balance + eye */}
              <div className="flex items-start justify-between">
                <span className="text-white font-bold text-lg tracking-wider uppercase">
                  {issuer}
                </span>
                <div className="flex flex-col items-end gap-0.5">
                  <p className="text-white/70 text-xs uppercase tracking-wider">{balanceLabel}</p>
              ...
              <div className="flex items-end justify-between">
                <div>
                  ...
                  <p className="text-white text-sm font-semibold tracking-widest mt-0.5 uppercase">
                    {holder}
                  </p>
                </div>
                <div>
                  <p className="text-white/70 text-xs uppercase tracking-wider">Expires</p>
                  <p className="text-white text-sm font-semibold tracking-widest mt-0.5">{expiry}</p>
                </div>
              ...
              <CardContent className="p-0 px-6 flex flex-col gap-3">
                <div className="flex items-end gap-3">
                  <div className="flex-1 h-8 bg-white/90 rounded-sm flex items-center overflow-hidden">
                    {Array.from({ length: 24 }).map((_, i) => (
                      <div
                        key={i}
                        className={cn("flex-1 h-full", i % 2 === 0 ? "bg-slate-300/70" : "bg-white/80")}
                      />
                    ))}
                  </div>
                  <div className="flex flex-col items-center gap-0.5 shrink-0">
                    <span className="text-white/70 text-xs uppercase tracking-widest">CVV</span>
                ...
                <div>
                  <p className="text-white/60 text-xs leading-relaxed">
                    This card is on file for your MockMatch subscription. Tap to flip back to the
                    front.
                  </p>
                  <div className="flex items-center justify-end">
                    <span className="text-white/70 text-xs">mockmatch.app</span>
                  </div>
                </div>
              </CardContent>
            </div>
