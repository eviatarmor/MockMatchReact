...
export function ShareDialog({
  ...
  const grantDev = trpc.collab.grantDevCredits.useMutation({
    onSuccess: () => {
      utils.collab.getAccess.invalidate({ kind, id: documentId }).catch(() => {})
      toast.success(t("share.devCreditsGranted"))
    },
  })
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[min(90vh,36rem)] w-full max-w-[calc(100%-2rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-md">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-4">
          ...
          ) : !isPaidOwner || !canShare ? (
            <div className="space-y-3 rounded-lg border border-dashed p-4 text-sm">
              <p className="text-muted-foreground">{t("share.paidRequired")}</p>
