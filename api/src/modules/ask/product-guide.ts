...
5. **Simulations** (\`/simulations\`) — recent practice **sessions** merge **voice** + **IDE** attempts (\`voice.listSessions\` + \`practiceSessions.list\`; each retake is its own row). Opening an unfinished IDE exercise offers **Continue** vs **Start new**. Featured **interview tracks** strip shows recent bank questions; **Browse all** opens \`/simulations/tracks\`. Tracks browse lists the **same global question bank** as Question Bank (\`questions.list\`) with the **same filters**: domain, difficulty, status. Practice routes match bank (conversation / mcq / whiteboard / \`/simulations/practice/:questionId\`). Seed IDE labs (\`practice_exercises\` + client \`INTERVIEW_TRACKS\`) remain for direct URLs and session history labels — not listed in the tracks table. **Formats**: **Code run**, **Dev workspace**, **Terminal lab**, **Conversation**, **MCQ**, **Whiteboard**, **Document** (freeform page).
   ...
   - **Document analysis** (\`/simulations/page\`) — freeform Notion/Docs-like Lexical page (\`@mockmatch/page\`): headings, lists, checklist, quote, code, slash menu; optional Yjs via document-editor plugin.
