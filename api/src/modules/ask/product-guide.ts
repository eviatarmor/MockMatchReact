...
### Practice area
5. **Simulations** (\`/simulations\`) — recent practice **sessions** merge **voice** + **IDE** attempts (\`voice.listSessions\` + \`practiceSessions.list\`; each retake is its own row). Opening an unfinished IDE exercise offers **Continue** vs **Start new**. Featured **interview tracks** strip shows recent bank questions; **Browse all** opens \`/simulations/tracks\`. Tracks browse lists the **same global question bank** as Question Bank (\`questions.list\`) with the **same filters**: domain, difficulty, status. Practice routes match bank (conversation / mcq / whiteboard / \`/simulations/practice/:questionId\`). Seed IDE labs (\`practice_exercises\` + client \`INTERVIEW_TRACKS\`) remain for direct URLs and session history labels — not listed in the tracks table. **Formats**: **Code run**, **Dev workspace**, **Terminal lab**, **Conversation**, **MCQ**, **Whiteboard**, **Spreadsheet**, **Document** (freeform page).
   ...
   - **Spreadsheet** (\`/simulations/spreadsheet\`) — freeform multi-sheet grid (\`@mockmatch/spreadsheet\`): virtualized cells, formula bar (HyperFormula), sheet tabs; durable collab kind \`spreadsheet\`.
   - **Document analysis** (\`/simulations/page\`) — freeform Notion/Docs-like Lexical page (\`@mockmatch/page\`): headings, lists, checklist, quote, code, slash menu; durable collab kind \`page\` (Yjs via document-editor plugin).
...
6. **Question Bank** (\`/question-bank\`) — **shared global content bank** (\`questions\` only): format + payload + \`content_cache\` / \`content_prefix\` for files. Auto-filled on apply/import. Kimi K3 + vector dedupe. **Formats generated:** conversation, code_run, **mcq** (whiteboard is practice-ready; generation optional). Practice: conversation → voice (\`@mockmatch/voice-agent\` shell + host Pipecat transport); **mcq** → \`/simulations/mcq/:seedId\` (same-domain **sequence**, IdeChromeBar + resizable rail; variants **single** / **multi** (select all) / **order** (dnd-kit reorder); check → next → summary; \`forMcqSession\` + \`submitMcq\`); **whiteboard** → \`/simulations/whiteboard/:questionId\` (\`@mockmatch/whiteboard\` shell: pan/zoom canvas, sticky/text/shapes/pen/connectors, templates gallery, autosave \`whiteboard.*\`, collab kind \`whiteboard\`); code_run/workspace/terminal → \`/simulations/practice/:questionId\` (IDE loads bank row). **Domains:** coding, systemDesign, dataScience, ml, security, devops, product, design, caseStudy, consulting, behavioral, finance, marketing, sales, clinical. Seeds stay in \`practice_exercises\`.
