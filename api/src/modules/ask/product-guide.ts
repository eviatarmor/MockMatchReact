...
6. **Question Bank** (\`/question-bank\`) — **shared global content bank** (\`questions\` only): format + payload + \`content_cache\` / \`content_prefix\` for files. Auto-filled on apply/import. Kimi K3 + vector dedupe. **Formats generated:** conversation, code_run, **mcq**. Practice: conversation → voice (\`@mockmatch/voice-agent\` shell + host Pipecat transport); **mcq** → \`/simulations/mcq/:seedId\` (same-domain **sequence**, IdeChromeBar + resizable rail; variants **single** / **multi** (select all) / **order** (dnd-kit reorder); check → next → summary; \`forMcqSession\` + \`submitMcq\`); code_run/workspace/terminal → \`/simulations/practice/:questionId\` (IDE loads bank row). **Domains:** coding, systemDesign, dataScience, ml, security, devops, product, design, caseStudy, consulting, behavioral, finance, marketing, sales, clinical. Seeds stay in \`practice_exercises\`.
